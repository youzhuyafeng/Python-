                        ...
                        while k <= 6:
                            try:
                                if k == 1:
                                    ...
                                    first_branch_response = get_response(url=all_url, params=next_branch_params, headers=headers)
                                    handle_branch_comment(first_branch_response)
                                    branch_max_id = get_max_id(first_response)
                                    if branch_max_id[0] == 0:
                                        break
                                    k += 1
                                    time.sleep(1)
                        ...
                        while k <= 6:
                            try:
                                if k == 1:
                                    ...
                                    first_branch_response = get_response(url=all_url, params=next_branch_params, headers=headers)
                                    handle_branch_comment(first_branch_response)
                                    branch_max_id = get_max_id(next_response)
                                    if branch_max_id[0] == 0:
                                        break
                                    k += 1
                                    time.sleep(random.uniform(0.5, 1))
                        ...
                        while k <= 6:
                            try:
                                if k == 1:
                                    ...
                                    first_branch_response = get_response(url=all_url, params=next_branch_params,
                                                                         headers=headers)
                                    handle_branch_comment(first_branch_response)
                                    branch_max_id = get_max_id(first_branch_response)
                                    if branch_max_id[0] == 0:
